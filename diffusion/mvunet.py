...
def get_camera_embedding(
    camera_para: torch.Tensor,    # [bs, len]
    embedding_dim: int,
    ...
    bs = camera_para.shape[0]
    half_dim = embedding_dim // 2
    exponent = -math.log(max_period) * torch.arange(
        start=0, end=half_dim, dtype=torch.float32, device=camera_para.device
    )
    exponent = exponent / (half_dim - downscale_freq_shift)

    emb = torch.exp(exponent)
    emb = camera_para[..., None].float() * emb[None, :]
...
class camera_embedding(nn.Module):
    def __init__(self, in_channels=320, out_channels=1280, flip_sin_to_cos=True, downscale_freq_shift=0):
        super(camera_embedding, self).__init__()
    ...
    def forward(self, x):
        # encoding
        # x = get_camera_embedding(x, self.embed_channels, self.flip_sin_to_cos, self.downscale_freq_shift)
        # embedding
        x = self.linear_1(x)
        x = self.act(x)
        x = self.linear_2(x)
...
class MVEpipolarAttnProcessor2_0:
    r"""
    Processor for implementing scaled dot-product attention (enabled by default if you're using PyTorch 2.0).
    """
    ...
    def __call__(
        ...
        scale: float = 1.0,
        num_view=None,
        epipolar_weights=None,
    ):
        if num_view is not None:
            if num_view != self.num_view:
                print(f"Call MV3DSelfAttnProcessor with num_view={num_view}!= self.num_view={self.num_view}! use num_view={num_view}.")
        else:
            num_view = self.num_view
        
        if epipolar_weights is None:
            raise ImportError("MVEpipolarAttnProcessor2_0 requires epipolar_weights as input!")
...
@dataclass
class MVUNet2DConditionOutput(BaseOutput):
    """
...
class MVUNet2DConditionModel(UNet2DConditionModel):
    r"""
    ...
    def __init__(
        ...
        print('initializing camera embedding MLPs ... ')
        # 21 * 20 == 420 dim, 12 * 20 = 240
        self.camera_projection = camera_projection(num_channels=20, flip_sin_to_cos=flip_sin_to_cos, downscale_freq_shift=freq_shift)
        self.camera_embedding = camera_embedding(in_channels=240, out_channels=1280)
    ...
    def from_unet(
        ...
        if load_weights_from_unet:
            ...
            unmatched = mvunet.load_state_dict(state_dict, strict=False)
            assert len(unmatched.unexpected_keys) == 0
            assert len(unmatched.missing_keys) == len(mvunet.mvunet_additional_keys)
            for k in unmatched.missing_keys:
                assert k in mvunet.mvunet_additional_keys
    ...
    def forward(
    ...
    ) -> Union[MVUNet2DConditionOutput, Tuple]:
        r"""
        The [`MVUNet2DConditionModel`] forward method.
        ...
        ## camera intrinsic & extrinsic embedding
        if camera_intrinsics is not None or camera_extrinsics is not None:
            if camera_intrinsics is None:
                camera_params = camera_extrinsics
            elif camera_extrinsics is None:
                camera_params = camera_intrinsics
            else:
                camera_params = torch.cat([camera_intrinsics, camera_extrinsics], dim=-1)
            cam_emb = self.camera_projection(camera_params)
            cam_emb = cam_emb.to(sample.dtype)
            cam_emb = self.camera_embedding(cam_emb)
            emb = emb + cam_emb
        ...
        return MVUNet2DConditionOutput(sample=sample)
