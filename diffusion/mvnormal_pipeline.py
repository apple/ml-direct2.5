...
class MVDiffusionPipelineOutput(BaseOutput):
    images: Union[List[PIL.Image.Image], np.ndarray]
    nsfw_content_detected: Optional[List[bool]]
    

class MVDiffusionPipeline(diffusers.StableDiffusionPipeline):
    ...
    def prepare_latents(
        ...
        if latents is None:
            ...
            noise = randn_tensor(shape, generator=generator, device=device, dtype=dtype)
            latents = self.scheduler.add_noise(latents, noise, self.scheduler.timesteps[0]).to(device)
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.unet(
                    ...
                    camera_intrinsics=torch.cat([camera_intrinsics] * 2) if camera_intrinsics is not None else None,
                    camera_extrinsics=torch.cat([camera_extrinsics] * 2) if camera_extrinsics is not None else None,
        ...
        return MVDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept)