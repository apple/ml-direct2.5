...
#
# https://github.com/Profactor/continuous-remeshing/blob/main/core/opt.py

from copy import deepcopy
import time
import torch
import torch_scatter
from meshing.remesh import calc_edge_length, calc_edges, calc_face_collapses, calc_face_normals, calc_vertex_normals, collapse_edges, flip_edges, pack, prepend_dummies, remove_dummies, split_edges
...
class MeshOptimizer:
    ...
    def remesh(self, flip:bool=True, valid_vol=None)->tuple[torch.Tensor,torch.Tensor]:
    ...
    def remove_unreferenced_vertices(self, vertices, faces) -> None:
        """
        Remove all vertices in the current mesh which are not
        referenced by a face, and update vertex indices accordingly.
        """
        referenced = torch.zeros(len(vertices)).bool().cuda()
        referenced[faces] = True

        inverse = torch.zeros(len(vertices)).long().cuda()
        inverse[referenced] = torch.arange(referenced.sum()).cuda()

        faces_update = inverse[faces]
        vertices_update = vertices[referenced]
        
        return vertices_update, faces_update
